###
# This file fixes the timestamp error when the CSV saved the file without zeros in teh us range. 
# It also adapts to data collected with different numbers of sensors.
###
...
class HiSTIFFSData:
    ...
    def repack_data(self, data):
        ...
        us = split_seconds[1].fillna('0').astype(np.int64)
    ...
    def estimate_all_stalks_stiffness(self, method='quasi-static average'):
        ...
        for i, stalk_trace_data in enumerate(zipped):
            ...
            # Estimate from C-A difference (requires C and A)
            if 'C' in self.sensor_labels and 'A' in self.sensor_labels and 'parallel_deflection' in self.data_dict['Sensor_C']:
                parallel_deflection = self.data_dict['Sensor_C']['parallel_deflection']
                estimate_1 = (sensor_readings['C'] - sensor_readings['A']) / parallel_deflection
                estimates_raw.append(estimate_1)
            
            # Estimate from C-E difference (requires C and E)
            if 'C' in self.sensor_labels and 'E' in self.sensor_labels and 'parallel_deflection' in self.data_dict['Sensor_C']:
                parallel_deflection = self.data_dict['Sensor_C']['parallel_deflection']
                estimate_2 = (sensor_readings['C'] - sensor_readings['E']) / parallel_deflection
                estimates_raw.append(estimate_2)
            
            # Estimate from B (angled sensor, requires B)
            if 'B' in self.sensor_labels and 'abs(yaw)_rad' in self.data_dict['Sensor_B']:
                yaw_B = self.data_dict['Sensor_B']['abs(yaw)_rad']
                estimate_3 = sensor_readings['B'] / np.sin(yaw_B) * np.cos(np.radians(38))
                estimates_raw.append(estimate_3)
            
            # Estimate from D (angled sensor, requires D)
            if 'D' in self.sensor_labels and 'abs(yaw)_rad' in self.data_dict['Sensor_D']:
                yaw_D = self.data_dict['Sensor_D']['abs(yaw)_rad']
                estimate_4 = sensor_readings['D'] / np.sin(yaw_D)
                estimates_raw.append(estimate_4)

            estimates_raw = np.array(estimates_raw) * beam_constant
            
            # Filter out NaN values
            estimates_filt = [e for e in estimates_raw if not np.isnan(e)]
            
            self.estimates.append(estimates_raw)
            self.stiffnesses[i] = np.average(estimates_filt) if estimates_filt else np.nan

        self.estimates = np.array(self.estimates, dtype=object)
    ...
    def save_stiffnesses(self, directory=None, note=None):
        ...
        # Dynamically create header based on number of estimates available
        max_estimates = max(len(est) for est in self.estimates) if len(self.estimates) > 0 else 0
        header = ['Stalk', 'Stiffness (N/m^2)'] + [f'Estimate {i+1}' for i in range(max_estimates)]
        ...
        with open(self.results_path, 'w', newline='') as f:
            ...
            for i in range(self.num_stalks):
                row = [i+1, self.stiffnesses[i]]
                for j in range(max_estimates):
                    if j < len(self.estimates[i]):
                        row.append(self.estimates[i][j])
                    else:
                        row.append('')  # empty cell if this stalk had fewer estimates
                writer.writerow(row)
...
if __name__ == "__main__":
    times1 = ['135058','135242','135350','135645','135858','140321','140422','140517','140559']
    times2 = ['104826','104944','105048','105207','105255','105341','105442','105526','105743']
    for i in range(2):
        if i == 0:
            times = times1
            date = "2026-07-14"
        ...
        for j in range(len(times)):
            data = HiSTIFFSData(date=date, time=times[j], debug=True)
            if data.exists:
