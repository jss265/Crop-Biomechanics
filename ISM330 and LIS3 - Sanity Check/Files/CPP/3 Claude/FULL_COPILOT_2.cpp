 ...
 *     IRAM ISR captures esp_timer_get_time() (1 us resolution) and gives a
 *     FreeRTOS notification to the IMU sensor task (pinned to core 0).  The
 *     task then bursts the whole FIFO out over SPI, demultiplexes the
 *     per-word tag byte into accel/gyro pairs, publishes a "latest sample"
 *     snapshot and bumps a counter for every pair drained.  This replaces
 *     the old sample-by-sample DRDY read: ~WTM/2 fewer interrupts.
 *   - LIS3MDL: the part has NO hardware FIFO, so it stays DRDY-driven.  Its
 *     DRDY pin is wired to a GPIO; the ISR timestamps + notifies the MAG
 *     task, which does a single 6-byte burst read of the output registers.
...
static uint32_t  magMissedCount = 0;   // DRDYs that arrived while busy
...
static void imuTask(void *) {
  // One FIFO word = 7 bytes (tag + 6 data).  Drain up to the full FIFO in a
  // single burst so we never leave words behind between notifications.
...
void setup() {
  ...
  xTaskCreatePinnedToCore(imuTask, "imu", 8192, nullptr,
  ...
  // attachInterrupt() was armed.  Notify the task once so it drains any
